...
argparser.add_argument('-p', '--port', dest='port',
                       help='IMAP port', default=143, type=int)
...
def process(mail, folder):
    ...
    for num in data[0].split():
        ...
        file = date + re.sub(r'(\<|\>|\$)', '', msg['Message-ID'])
...
def main():
    ...
    mail = imaplib.IMAP4(args.server, args.port)
    ...
    try:
        ...
        print("mail.list([0])"+" = "+mail.list()[0])

        print("mail.list()[1] = "+str(mail.list()[1]))
